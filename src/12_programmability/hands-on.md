...
1. 参考 `src/11_patterns/code/patterns_lab/` 中的 `fun init`（可与第十一章 · 设计模式对照阅读）。
2. 在 `programmability_lab` 或副本包中，为模块添加 `init`，只做**一件**事（如发一个 `AdminCap` 给部署者）。
3. **验收**：`sui move test` 或本地构建通过；能解释 `init` 何时运行一次、升级时为何不再次运行。

## 实战三：Clock 只读调用（阶段 C）

1. 阅读 [§12.5 · Epoch 与时间](05-epoch-and-time.md) 与 `src/21_advanced_topics/code/advanced_lab/sources/clock_probe.move`（若仓库中有）。
