...
- **后续**：[第九章 §9.5 · 内部约束](../09_using_objects/05-internal-constraint.md)（为何不能 `emit` 外模块类型）  
...
注意：事件类型**不能**使用从其他模块导入的类型作为事件 emit。你只能 emit 当前模块中定义的结构体。该要求来自 Sui 验证器的**内部约束**规则，原理与更多示例见[内部约束](../09_using_objects/05-internal-constraint.md)。
