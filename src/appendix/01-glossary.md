...
| 术语 | 英文 | 解释 |
|------|------|------|
| （旧称）快速路径 | Fast Path（deprecated） | 教材不再使用该机制名；见第 8 章 §7.4 与[官方架构说明](https://docs.sui.io/concepts/sui-architecture) |
...
| 幻影类型参数（`phantom`） | Phantom Type Parameter | 未出现在结构体字段中的类型参数，须写 `phantom T`；见[第七章 §7.3](../07_move_advanced/03-phantom-type-parameters.md) |
...
| TreasuryCap | TreasuryCap | 铸币权对象，内含 `Supply<T>`，是 `mint`/`burn` 的正规入口；见[第十二章 §12.11](../12_programmability/11-balance-and-coin.md)、[第十四章](../14_tokens/00-index.md) |
