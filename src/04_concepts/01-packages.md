...
`init` 在语言层面的要点（只记结论即可，写法与模式详见 **[第十三章 §13.3 · 模块初始化器 init](../13_programmability/03-module-initializer.md)**）：
...
| 返回值 | **无** |
| 参数 | **一种**：`ctx: &mut TxContext`；**或两种**：一次性见证类型 **`OTW` + `ctx`**（OTW 名称须与模块名对应，见第十三章与[第十二章 · OTW](../12_patterns/03-one-time-witness.md)） |
